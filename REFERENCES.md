...
> - **API4:2023 – Unrestricted Resource Consumption:** Kong rate-limiting plugin cấu hình trong `gateway/kong.yml`; policy bổ sung tại `opa/policies/rate_limit.rego`.  
...
> **Áp dụng:** Tham chiếu cho thiết kế middleware xác thực backend (`backend/app/middleware/auth_middleware.py`) và cấu hình bảo mật PostgreSQL (AES-256-GCM at-rest).
...
> **Áp dụng:** Authorization Code Flow (user-facing) và Client Credentials Flow (S2S) được cấu hình trong Keycloak realm (`idp/keycloak/realm-export.json`); token endpoint `POST /realms/cloudapi/protocol/openid-connect/token`.
...
> **Áp dụng:** PKCE (S256 method) bắt buộc cho public clients trong Keycloak; `code_challenge` và `code_verifier` xử lý tại frontend `frontend/src/auth/keycloak.js` (`AuthProvider.jsx`); bảo vệ chống authorization code interception attack.
...
> **Áp dụng:** DPoP proof verification tại `backend/app/security/dpop_verifier.py`; `jti` (JWT ID) lưu vào Redis `SET NX + TTL` để chống replay attack; kiểm thử bằng `scripts/attacks/replay_dpop_attack.py` → `401` (invariant I4 — `EVAL/E-N2.md`). Deployment D2 thay thế bằng mTLS east-west.
...
> **Áp dụng:** OIDC flow hoàn chỉnh qua Keycloak v24.0: Discovery endpoint (`/.well-known/openid-configuration`), `id_token` validation, `UserInfo` endpoint, JWKS rotation; cấu hình tại `idp/keycloak/`.
...
> **Áp dụng:** TOTP (HMAC-based OTP, AAL2) triển khai tại `backend/app/security/totp_verify.py` với thư viện `pyotp==2.9.0`; refresh token rotation + reuse-detection (Keycloak); session cookie `Secure + HttpOnly + SameSite`; kiểm thử tại `scripts/evaluation/e_n1_totp_test.py` (invariant I4).
...
> **Áp dụng:** Envelope encryption KEK/DEK (AES-256-GCM) qua HashiCorp Vault Transit Engine (`vault/init/enable-transit.sh`); key rotation SLA ≤ 10 phút (`scripts/evaluation/e_x1_rotation_test.sh`); blast-radius ≤ 24h (invariant I6 — `EVAL/E-X1.md`, `EVAL/E-X2.md`).
...
> **Vai trò trong project:** Policy Enforcement Point (PEP); JWT verification (`jwt-hardening.lua`), rate limiting, WAF, CORS, HSTS header injection, OPA authorization sidecar (`opa-authz.lua`). Cấu hình declarative tại `gateway/kong.yml` và `gateway/deck/kong-declarative.yml`.

**[TOOL-2] Keycloak v24.0**  
Red Hat / Keycloak Community. *Keycloak — Open Source Identity and Access Management*. 2024.  
URL: https://www.keycloak.org/documentation  
> **Vai trò trong project:** Identity Provider (IdP); OIDC/OAuth 2.0 Authorization Server; PKCE flow, Client Credentials flow, refresh token rotation với reuse-detection, TOTP MFA, JWKS endpoint cho Kong fetch. Cấu hình realm tại `idp/keycloak/realm-export.json`.
...
> **Vai trò trong project:** Key Management Service (KMS); Transit Secrets Engine cho envelope encryption (KEK/DEK); key rotation API (`/v1/transit/keys/dek/rotate`); policy DEK tại `vault/policies/dek-policy.hcl`; init script tại `vault/init/enable-transit.sh`. Invariant I6: rotation ≤ 10 phút.

**[TOOL-4] Open Policy Agent (OPA) v0.65.0**  
CNCF / Styra. *Open Policy Agent — Policy-as-Code*. 2024.  
URL: https://www.openpolicyagent.org/docs/latest/  
> **Vai trò trong project:** Policy Decision Point (PDP); Rego policies tại `opa/policies/` (authz, admin, rate_limit); deny-by-default, RBAC→ABAC; 100% decision logging (invariant I5). Tích hợp với Kong qua `opa-authz.lua`.
...
> **Vai trò trong project:** Backend API framework; middleware xác thực (`auth_middleware.py`), DPoP verifier, AEAD encryption/decryption, TOTP verify, structured JSON logging.
...
> **Áp dụng:** TLS 1.3 enforce tại Kong edge và NGINX (D2); 0-RTT tắt; ciphersuites thu gọn (TLS_AES_256_GCM_SHA384, TLS_CHACHA20_POLY1305_SHA256); HSTS header `max-age=31536000; includeSubDomains`. Kiểm tra không rò rỉ plaintext: `scripts/evaluation/e_c1_tls_capture.sh` → `EVIDENCE/captures/tls_capture.pcap` (invariant I1).
...
| **Công cụ/Stack** | 5 | Kong, Keycloak, HashiCorp Vault, OPA, FastAPI |
